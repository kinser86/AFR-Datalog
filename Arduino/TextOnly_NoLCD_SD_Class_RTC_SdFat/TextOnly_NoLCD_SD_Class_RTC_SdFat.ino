...
void read(){
  //Read Values
  TPS_RAW = analogRead(PIN_TPS);
  AFR_RAW = analogRead(PIN_AFR);

  //Calculate Values
  TPS_VAL = (TPS_RAW * (4.0 / 1023.0)) + 0.5;    // TPS ADC
  //TPS_VAL = ((TPS_RAW * 5.0) / 1023.0);    // Voltage (VDC)
  AFR_VAL = (AFR_RAW * (10.0 / 1023.0)) + 10.0;  // AFR ADC
...
void write(){
  ...
  if (dataFile) {
    ...
    dataFile.print(",");
    dataFile.print(TPS_RAW);
    dataFile.print(",");
    dataFile.println(AFR_RAW);
  }
